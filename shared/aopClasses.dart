//
//  Model classes for All Our Photos generated by dartBO.txt on 18-Oct-2019 13:09:32 by chris
//
import 'dart:async';
...
Map<String,dynamic> toMap() {
  ...
  List<dynamic> toRow({bool insert=false}) {
	...
	result.add(dbDate(firstDate?.toLocal()));
	result.add(dbDate(lastDate?.toLocal()));
	result.add(_userId);
  return result;
}  // to Row
 @override 
...
Map<String,dynamic> toMap() {
  ...
  List<dynamic> toRow({bool insert=false}) {
	...
	if (insert)
		result = super.toRow();
	result.add(dbDate(startDate?.toLocal()));
	result.add(dbDate(endDate?.toLocal()));
	result.add(source);
	result.add(_userId);
  return result;
}  // to Row
...
Map<String,dynamic> toMap() {
  ...
  List<dynamic> toRow({bool insert=false}) {
	...
	result.add(dbDate(takenDate?.toLocal()));
	result.add(dbDate(originalTakenDate?.toLocal()));
	result.add(dbDate(modifiedDate?.toLocal()));
	...
	result.add(dbDate(importedDate?.toLocal()));
