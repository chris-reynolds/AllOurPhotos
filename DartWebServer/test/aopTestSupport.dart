...
const String TEST_HOST = 'http://localhost:4444';
...
class TestRequest {
  ...
  Future<TestRequest> get(String url,
      {String? accept, String? cookie, List<int>? putData}) async {
    ...
    if (cookie != null) request.cookies.add(Cookie('aop', cookie));
    response = await request.close();
    responseText = await response!.transform(utf8.decoder).toList();
    testLog(
        'Response ${response!.statusCode}: type ${responseText.runtimeType} $responseText');
...
//    print('headers: ${response.headers}');
    expect(response!.headers.value(expectedKey), contains(expectedText),
        reason: 'Header ' + expectedKey);
  } // of expectHeader
