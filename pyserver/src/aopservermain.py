...
from fastapi import FastAPI, HTTPException, Request, Response, File, UploadFile, Query
...
async def makeSession(response: Response, user,password,source):
    user     = re.sub(r'[^a-zA-Z0-9]', '', user)[:20]
    password = re.sub(r'[^a-zA-Z0-9]', '', password)[:20]
    source   = re.sub(r'[^a-zA-Z0-9]', '', source)[:20]
    rows = raw_sql(f"select spsessioncreate('{user}','{password}','{source}') as sessionid")
...
#    (updated_user,user_id) = username_and_id(request)
    ...
    values = []
    for k in valueMap:
        sql = sql.replace("'@" + k + "'", '%s')  # quoted token → unquoted %s (connector handles quoting)
        sql = sql.replace('@' + k, '%s')          # unquoted token (numeric etc.)
...
async def upload_video(request: Request, modified: str, filename: str, sourceDevice: str, video: UploadFile = File(...)):
    try:
        ...
        if creation_time:
            ...
            taken_datetime = datetime.strptime(modified, '%Y:%m:%d %H:%M:%S')
...
def create_albumItem(request:Request, albumItem: AlbumItem) -> AlbumItem:
    try:
        ...
        if hasattr(albumItem,'user_id'): albumItem.user_id = user_id  # type: ignore[attr-defined]
...
def create_user(request:Request, user: User) -> User:
    try:
        ...
        if hasattr(user,'user_id'): user.user_id = user_id  # type: ignore[attr-defined]
