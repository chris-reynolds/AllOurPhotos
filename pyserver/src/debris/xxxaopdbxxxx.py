...
class DBSession():
    ...
    def fetch(self,entityName:str, where: str='1=0', orderby: str='id', one:bool = False, columns:str=''):
        if not self.entityIsValid(entityName): 
            raise Exception('out of scope')
        sqltext = self.sqlFactory.selectStatement(entityName,columns)
        sqltext += f" where {where} order by {orderby}"
    ...
    def makeSession(self,user:str,password:str,source:str):
        ...
        if row is not None:
          self.id = row[0]  # todo : just get first column
        acursor.close()
...
class SqlFactory():
    ...
    def selectStatement(self,entityName: str,overrideColumns: str):
        if not entityName in self._metadata:
            return ''
        if overrideColumns != '':
            return f"select {overrideColumns} from {entityName}"
