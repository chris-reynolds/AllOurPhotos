...
function loadConfig() {
  let result = {
    ...
    clientDir : 'C:\\projects\\OurPhotos\\nev\\client',
    imagesDir : 'C:\\projects\\OurPhotos\\nev\\testdata',
    realDir : 'P:\\photos'

  }
  if (fs.existsSync('config.json'))
    result = _.merge(result,JSON.parse(fs.readFileSync('config.json')));
...
class FSDirectory {
  ...
  getDirectory(dirName) {
    let idx = _.findIndex(this.directories(),['_root',dirName])
  ...
  getYear(selectedYear) {
    ...
    this.directories().forEach(function (directory) {
      let thisYear = directory._root.substr(0,4)
      let thisMonth = directory._root.substr(5,2);
      if (thisYear==selectedYear && result.indexOf(thisMonth)==-1)
        result.push(thisMonth)
    }) // of foreach
    return {'year':selectedYear,'months': result.sort()}
  } // of getYear
