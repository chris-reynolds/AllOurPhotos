...
//        var db = DbAllOurPhotos();
        success = await uploadAsset(_images[imageIx]);
      } catch (ex) {
        success = false;
...
//  }
  ...
  Future<bool> uploadAsset(MIP.Asset anImage) async {
...
//      print('Running on ${iosInfo.utsname.machine}');  // e.g. "iPod7,1"
   ...
   List<int> imageContents = fullImageBytes.buffer.asUint32List().toList();
    return await uploadImage(imageContents, anImage.name,device: deviceName);
  } // uploadAsset

//  Future<bool> uploadImageX(MIP.Asset anImage, DbAllOurPhotos db) async {
//    try {
//      DeviceInfoPlugin deviceInfo = DeviceInfoPlugin();
//      String deviceName = 'Unknown';
//      if (!Platform.isIOS) {
