...
void main() async {
  ...
  addToFile('metrics.log',
      ',$loc,${taskList.length},$totalTaskSize,$totalTaskOutstanding,${bugList.length},$bugOsCount,0,0,0,0');
...
double timeSpentRatio() =>
    DateTime.now().difference(STARTDATE).inDays / PROJECTDURATION;
...
class Bug {
  ...
  Bug.fromLine(List<String> thisLine) {
    for (int i = 0; i < thisLine.length; i++) thisLine[i] = thisLine[i].trim();
    number = thisLine[0];
    description = thisLine[1];
    if (thisLine.length > 2 && thisLine[2].length > 0) severity = thisLine[2];
    if (thisLine.length > 3) created = DateTime.parse(thisLine[3]);
...
class Task {
  ...
  String progress =
      'U'; // unstarted,inprogress,designing,programming,testing,finished
  String group = '';
  DateTime? finishedOn;
  ...
  Task.fromLine(List<String> thisLine) {
    ...
    if (thisLine.length > 4) group = thisLine[4];
  ...
  static void saveCarPark(List<Task> taskList) {
    ...
    for (var task in taskList) {
      if (task.group.length > 0 || task.sizeInUnits > 0) {
        ...
        for (int sectionIx = 0; sectionIx < sections.length; sectionIx++)
          if (sections[sectionIx][0] == task.group) foundSectionIx = sectionIx;
        if (foundSectionIx == -1) {
        ...
        sections[foundSectionIx][2] +=
            task.sizeInUnits - task.outstandingInUnits;
    ...
    for (var section in sections)
      logFile.writeln('${pad(section[0], 12)}' +
          done.substring(0, (section[2]).round()) +
          todo.substring(0, (section[1] - section[2]).round()) +
          pad('', (60 - section[1].round()).toInt()) + //section[1].round()) +
          displayPair(section[2], section[1]));
    logFile.writeln();
    logFile.writeln(
        '${pad('              ', 72)}' + displayPair(totalDone, totalSize));
    logFile.writeln(
        'Velocity = ${((totalDone / totalSize) * 100 / timeSpentRatio()).round()}% ' +
...
void addToFile(String filename, String contents, {bool timestamp = true}) {
  ...
  if (timestamp) _timestampStr = DateTime.now().toString() + ' ';
  try {
  ...
  for (int ix = 0; ix < tempList.length; ix++) {
    ...
    if (terminator > 0)
      tempList[ix] = tempList[ix]
          .substring(terminator + 2); // todo check for contig multi comments
  }
